...
class UserResponse(BaseModel):
    ...
    class Config:
        json_encoders = {
            UUID: str  # Automatically convert UUID to string when serializing to JSON
        }
...
def signup(user_create: UserCreate, session: Session = Depends(get_session)) -> UserResponse:
    ...
    try:
        ...
        # Generate JWT token - convert UUID to string for serialization
        payload = {
            "user_id": str(db_user.id),  # Convert UUID to string
        ...
        if any(keyword in error_msg for keyword in ['connection', 'could not receive data', 'ssl syscall', 'connection abort']):
            raise HTTPException(
                status_code=503,
